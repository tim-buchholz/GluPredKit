...
class Parser(BaseParser):
    ...
    def __call__(self, start_date, end_date, username: str, password: str, save_raw_json=False):
        ...
        try:
            ...
            api_start_ms = datestring_to_epoch_ms(api_start_date)
            api_end_ms = datestring_to_epoch_ms(api_end_date)

            # Fetch entries (CGM data)
            query_params = {
                "count": 0,
                "find[date][$gte]": api_start_ms,
                "find[date][$lte]": api_end_ms,
            }
                ...
                if event_type:
                    ...
                    if hasattr(entry, 'dateString'):
                        ...
                        if isinstance(date, (int, float)):
                            date_value = pd.to_datetime(date, unit='ms', utc=True)
                        else:
                            date_value = pd.to_datetime(date, utc=True)
                    else:
